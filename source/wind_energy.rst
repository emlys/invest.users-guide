...
The wind energy model estimates wind power density (wind power potential) to identify offshore areas with high energy potential. The wind power density :math:`PD` (:math:`W\cdot m^{-2}`) at a certain location can be approximated as a function of wind statistics (Elliott et al., 1986)

.. math:: PD = \frac{1}{2}\rho\sum^c_{j=1}f(V_j)V_j^3
   :label: wind_power_density

where

* :math:`\rho` is mean air density (:math:`kg\cdot m^{-3}`)
* :math:`j` is the index of wind speed class
* :math:`c` is the number of wind speed classes
* :math:`V_j` is wind speed of the jth class (:math:`m\cdot s^{-1}`)
* :math:`f(V_j)` is probability density function of :math:`V_j`.
...
The value of wind power is measured as the discounted pre-tax net revenue from power generation that would accrue to a wind power developer/operator over the expected lifetime of a wind farm.
